...
if(isset($_POST['confirm'])) {
    ...
    $delete=$pdo->prepare('DELETE FROM orders WHERE order_id = :order_id');
    $values=[
        'order_id' => $_POST['order_id']
    ];
...
<form action="my-account.php" method="POST" class="">
    <?php
        $select = $pdo->prepare('SELECT * FROM orders WHERE order_status = "Pending" GROUP BY order_id ORDER BY order_date DESC');
        $select->execute();	

        echo '<label>Pending Orders:</label></br><select class="w-50 mb-3" name="order_id">';
    ...
    <input class="mx-3" type="submit" name="confirm" value="Confirm order">
